...
func (h *ResultHandler) HandleStockResult(msg rabbitmq.Message) error {
	...
	txn, err := h.txnRepo.FindByID(ctx, msg.TransactionID)
	if err != nil {
	...
	switch operation {
	case "DECREASE":
		return h.handleBorrowResult(ctx, txn, msg)
	case "INCREASE":
		return h.handleReturnResult(ctx, txn, msg)
	}

	return nil
}

func (h *ResultHandler) handleBorrowResult(ctx context.Context, txn *domain.BorrowTransaction, msg rabbitmq.Message) error {
	...
	case rabbitmq.EventTypeDecreaseStockSucceeded:
		...
		if err := h.txnRepo.ActivateBorrow(ctx, txn.ID, msg.EventID); err != nil {
			return fmt.Errorf("failed to activate borrow from stock result: %w", err)
		}
		return h.createAudit(ctx, txn.ID, string(domain.TransactionPending), string(domain.TransactionActive), "Borrow activated after stock reservation", msg.ErrorMessage)
	case rabbitmq.EventTypeDecreaseStockRejected:
		...
		if err := h.txnRepo.CancelBorrow(ctx, txn.ID); err != nil {
		...
		return h.createAudit(ctx, txn.ID, string(domain.TransactionPending), string(domain.TransactionCancelled), reason, msg.ErrorMessage)
	default:
		return fmt.Errorf("unsupported borrow stock result event type: %s", msg.EventType)
	}
}

func (h *ResultHandler) handleReturnResult(ctx context.Context, txn *domain.BorrowTransaction, msg rabbitmq.Message) error {
	switch msg.EventType {
	case rabbitmq.EventTypeIncreaseStockSucceeded:
		...
		if err := h.txnRepo.FinalizeReturn(ctx, txn.ID, msg.EventID); err != nil {
			return fmt.Errorf("failed to finalize return from stock result: %w", err)
		}
		updatedTxn, err := h.txnRepo.FindByID(ctx, txn.ID)
		if err != nil {
			return fmt.Errorf("failed to reload finalized return transaction: %w", err)
		}
		return h.createAudit(ctx, updatedTxn.ID, string(domain.TransactionReturnPending), string(updatedTxn.Status), "Return completed after stock restore", msg.ErrorMessage)
	case rabbitmq.EventTypeIncreaseStockRejected:
		...
		if err := h.txnRepo.RejectReturn(ctx, txn.ID); err != nil {
		...
		return h.createAudit(ctx, txn.ID, string(domain.TransactionReturnPending), string(domain.TransactionActive), reason, msg.ErrorMessage)
	default:
		return fmt.Errorf("unsupported return stock result event type: %s", msg.EventType)
	}
}

func (h *ResultHandler) createAudit(ctx context.Context, transactionID, fromStatus, toStatus, reason, metadata string) error {
	var from *string
	if fromStatus != "" {
		from = &fromStatus
	}
	...
	return h.auditRepo.Create(ctx, &domain.TransactionAudit{
		ID:            uuid.NewString(),
